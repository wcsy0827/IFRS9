...
namespace Transfer.Models.Repository
{
    public class Common : ICommon
    {
        ...
        public static IFRS9DBEntities DeleteToContext<T>(IFRS9DBEntities context,
            T entity, int count, int commitCount, bool recreateContext) where T : class
        {
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                ...
                if (//當 fileName,checkName 都為 A41 不用檢查(為最先動作)
                    ...
                    !checkTable.Any(x => x.File_Name == fileName &&
                                                  ...
                                                  x.TransferType == "Y"))
                    return true;
                //190222增加判斷A59Apex以及A59Tran記錄
                else if (fileName == Table_Type.A59Apex.ToString() &&
                    checkTable.Any(x => x.File_Name == fileName &&
                    x.ReportDate == reportDate &&
                    x.Version == version &&
                    x.TransferType == "Y"))
                ...
                else if (fileName == Table_Type.A59Trans.ToString() &&
                    checkTable.Any(x => x.File_Name == fileName &&
                    x.ReportDate == reportDate &&
                    x.Version == version &&
                    x.TransferType == "Y"))
            ...
            dbModel.ToList().ForEach(db =>
            {
                ...
                foreach (var item in viewpros)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        else if (_pt == typeof(TimeSpan) || _pt == typeof(Nullable<TimeSpan>))
                        {
                            if (p.GetValue(db) != null)
                                item.SetValue(obj, TypeTransfer.objTimeSpanToString(p.GetValue(db)));
                            else
                                item.SetValue(obj, string.Empty);
                        }
                        else
        ...
        public string getUserAccountAndName(IEnumerable<IFRS9_User> users, string account)
        {
            var _user = users.FirstOrDefault(x => x.User_Account == account);
            if (_user != null)
                return $"{account}({_user.User_Name})";
            return account;
        }
        ...
        public List<SelectOption> getDebtSelectOption(string account)
        {
            ...
            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                ...
                if (_user != null)
                {
                    result.AddRange(getDebt(_user.DebtType)
        ...
        public class User
        {
            public User()
            {
                _user = Controllers.AccountController.CurrentUserInfo.Name;
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        public List<string> GetDatabaseFieldName(string data)
        {
            List<string> result = new List<string>();

            using (IFRS9DBEntities db = new IFRS9DBEntities())
            {
                result = db.Database.SqlQuery<string>("SELECT COLUMN_NAME FROM IFRS9.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME ='" + data + "'").ToList();
            }
            return result;
        }
    }
